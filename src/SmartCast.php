...
class SmartCast
{
     ...
     */
    public static function stringToInt(string|int $value, ?NumberSign $sign = null, bool $strictType = true, bool $acceptsZero = true): int
    {
     ...
     */
    public static function stringToFloat(string|float $value, ?NumberSign $sign = null, bool $strictType = false, bool $acceptsZero = true): float
    {
        static::checkIsNumeric($value);
    ...
    private static function checkZeroValue(int|float $value, bool $acceptsZero): void
    {
        if (!$acceptsZero && $value == 0) {
            throw new ZeroValueException((string) $value);
        }
    }
}
